...
- [done] You will add functionality to all components inside inside `src/components`.
- [done] Work on the components **in the same order in which they display in Chrome** (to go from easiest challenge to hardest).
- [done] Each file includes a link to a video, and a set of instructions which can be summarized as:
